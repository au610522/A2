...
#Our best model - fitting the model
m10 <- lmer(data = train, 
     CHI_MLU ~ 1 + Visit * Diagnosis + Visit * MOT_MLU + Visit * verbalIQ1 + (1|Child.ID) + (0 + Visit|Child.ID))

# Basic model - since the powerSim does not work at the moment 
m0 <- lmer(data = train, 
     CHI_MLU ~  Diagnosis + Visit + (1|Child.ID))

#assissing the power of our interactions 
# needs to be above 80% 
powerSim(m0, test=fixed('Visit'), nsim = 50) # effect size = 0.23, power of 100% 
powerSim(m0, test=fixed('Diagnosis'), nsim = 50) # power of 96% 
# powerSim(m0, test=fixed('Visit:Diagnisis'), nsim = 50)

# using powerCurve
m2 <‐ extend(m0, along="Child.ID", n=15)

?powerCurve

pc1 <‐ powerCurve(m2, along="Child.ID")
