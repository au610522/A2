...
merge_train$Gender[merge_train$Gender == "2"] <- "F"
merge_train$Gender[merge_train$Gender == "1"] <- "M"
...
write.csv(merge_train, file = 'train.csv', row.names = FALSE)
```
...
utt_over_2.7 <- filter(merge_train, CHI_MLU > 2.7) %>%
  group_by(., SUBJ) %>%
  summarise(., n())


utt_under_1.5 <- filter(merge_train, CHI_MLU < 1.5 & VISIT == 1) 
  
all_trials <- group_by(merge_train, SUBJ) %>%
  summarise(., 'number'=n()) %>%
  filter(., number < 6)
...
```{r}
six_visit <- filter(merge_train, VISIT == 6) %>%
  arrange(., desc(tokens_CHI)) #desc makes it go from the biggest nr to the smallest - not writing anything starts from 0

first_visit <- filter(merge_train, VISIT == 1) %>%
  arrange(., tokens_CHI)
...
```{r}
small_train <- select(merge_train, SUBJ, Diagnosis, CHI_MLU, tokens_CHI) %>%
  filter(., Diagnosis == 'ASD')

### It only comes up one time even thought you write it more times 
```
...
```{r}
M <- mean(merge_train$tokens_CHI)

merge_train$mean_all <- M 

M_CHI <- group_by(merge_train, SUBJ) %>% 
  summarise(., mean_chi = mean(tokens_CHI))

merge_all <- merge(merge_train, M_CHI)

# does not take the diagnosis into acount 
# does not take the development of each child into acount 
```
